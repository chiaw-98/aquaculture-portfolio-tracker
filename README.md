...
A Streamlit dashboard tracking structural change in global aquaculture from an animal welfare perspective. It answers three questions:
...
- How much has each country's species mix shifted since 2019?
...
streamlit_datasheet.R     Master data pipeline — produces the Excel for Google Sheets
requirements.txt          Python dependencies

data/
  FAO_allyears_soc.csv                              Raw FAO production data with welfare flags
  nei/
    NEI_allocated_2000-2023_ALL_SCENARIOS_OPTIONS.csv   NEI-allocated dataset (three scenarios)
    [NEI allocation scripts]

archive/
  jaccard_step1.R           Standalone Jaccard script (superseded by streamlit_datasheet.R)
  StreamlitENS.R            Standalone ENS script (superseded by streamlit_datasheet.R)
  species_relative_growth_v3.R   Standalone species growth script (superseded)
```
...
The Streamlit app reads directly from the published Google Sheets CSV URLs, so no redeployment is needed if the URLs haven't changed. If you updated the URLs in step 3, push to GitHub — Streamlit Cloud will redeploy automatically.
